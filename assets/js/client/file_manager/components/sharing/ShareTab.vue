<template>
  <div class="card-body">
    ...
    <small class="text-muted" style="font-style: italic"
      >Remember to press TAB after each email address</small
    >
    <div class="buttons" style="float: right">
      <a
        class="btn btn-sm btn-primary pull-right mt-2"
        @click="shareFiles()"
      ...
      <a class="btn btn-sm btn-secondary pull-right mt-2" @click="hideModal()">
...
export default {
    ...
    shareFiles() {
      ...
      $.post(`/api/v1/shareable`, this.postFormData)
        ...
        .catch((e) => {
          this.$swal(
            "Error",
            "Unable to share files. Try again later.",
            "error"
          );
        });
    },
  ...
  computed: {
    ...
    showEmailError() {
      return (
        this.submit &&
        (this.emails.length === 0 || this.invalidEmails.length > 0)
      );
    },
    showLastNameError() {
    ...
    showFirstNameError() {
      return (
        this.submit && (this.firstName === "" || this.firstName.length <= 1)
      );
    },
    ...
    emailErrorMessage() {
      if (this.emails.length === 0 && this.submit) {
        ...
        return `${value} ${this.invalidEmails
          .map((e) => e.email)
          .join(", ")} ${is} not valid email ${address}`;
      }
    },
  },
  updated() {
