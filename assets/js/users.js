...
const loadUsersDataTable = () => {
    Datatable.initializeWithColumns('usersDT', 'table#users-datatable', [
      ...
      { "data": "roles", "name": "roles", "autoWidth": true, "sortable": false,
        "render" : function (data, type, object, meta)  {
          let buttons = `<button data-remote-details="true" type="button" data-url="/users/${object.id}" data-modal-title="${object.first_name} ${object.last_name}'s details" class="btn btn-xs btn-outline-info rounded-circle" data-toggle='tooltip' data-title='User details and statistics'><i class='fa fa-eye'></i></button>
